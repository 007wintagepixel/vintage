    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <header className="pt-32 pb-12 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          >
            From solo practice to competitive tournaments — every mode crafted
            for fair, exciting gameplay.
          </motion.p>
        </div>
      </header>
            ...
            {gameModes.map((mode, index) => (
              ...
              >
                ...
                <div className="relative p-8 h-full flex flex-col">
                  ...
                  {/* Title & Description */}
                  <h3 className="font-display text-heading-lg text-center mb-3">
                    {mode.title}
                  </h3>
                  <p className="text-text-secondary text-center text-body mb-8 flex-1">
                    {mode.description}
                  </p>
            ...
            <table className="w-full glass-card-strong rounded-2xl overflow-hidden">
              <thead>
                <tr className="bg-surface-tertiary/50 border-b border-surface-border">
                  <th className="px-6 py-4 text-left font-medium text-text-primary">
                    Feature
                  </th>
                  {gameModes.map((mode) => (
                    <th
                      key={mode.id}
                      className="px-4 py-4 text-center font-medium text-text-primary"
                    >
                      {mode.title}
                    </th>
                  ))}
              ...
              <tbody>
                {comparisonRows.map((row) => (
                  <tr
                    key={row.feature}
                    className="border-b border-surface-border/50 transition-colors hover:bg-surface-tertiary/30"
                  >
          ...
          <motion.div className="glass-panel-strong p-12 md:p-16 rounded-3xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-body-lg text-text-secondary mb-8 max-w-xl mx-auto">
                Pick a mode, invite friends, or test your skills against AI.
                Your first game is just a click away.
              </p>
              <Link
                href="/register"
                className="btn-primary px-10 py-4 text-body-lg inline-flex group"
              >
...
const comparisonRows = [
  ...
  },
  {
    ...
    values: ["1-4", "2-4", "2-4", "8-256", "4 (2v2)", "2-4"],
  },
  {
    feature: "Skill Matchmaking",
...
const coreRules = [
  {
    ...
    description:
      "Each player starts with 4 tokens in their home base. All must reach the center to win.",
  },
  {
    title: "Enter on Six",
    description:
      "A token leaves home only when you roll a 6 (configurable). Rolling 6 grants an extra turn.",
  },
  {
    title: "Capture Opponents",
    description:
      "Land on an opponent's token to send it back to their home. Capturing grants an extra turn.",
  },
  {
    title: "Safe Cells Protected",
    description:
      "Colored safe cells (start positions and 4 others) protect tokens from capture.",
  },
  {
    title: "Exact Roll to Finish",
    description:
      "Tokens must roll the exact number to enter the final home position. Overshooting cancels the move.",
  },
  {
    title: "Three Sixes Rule",
    description:
      "Rolling three 6s in a row cancels the turn and passes to the next player (configurable).",
  },
  {
    title: "Blockades Optional",
    description:
      "Two tokens of same color on one cell can form a blockade that cannot be passed (optional rule).",
  },
  {
    title: "Turn Timeout",
    description:
      "Each turn has a time limit (default 30s). Auto-move triggers if only one legal move exists.",
  },
];
