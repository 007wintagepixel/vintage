...
export default function LeaderboardPage() {
  ...
  const getMedalStyle = (rank: number) => {
    switch (rank) {
      case 1:
        return {
          bg: "from-secondary-glow to-amber-600",
          ring: "ring-secondary-glow/50",
          text: "text-secondary-glow",
          label: "Gold",
        };
      case 2:
        return {
      ...
      default:
        return {
          bg: "from-primary-glow to-accent-magenta",
          ring: "ring-primary-glow/30",
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-text-secondary mt-1">
            Climb the ranks and claim your throne
          </p>
        </div>
        <div className="glass-card p-4 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <div className="font-display text-heading-md text-primary-glow">
              #{yourRank?.rank}
            </div>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <div
            className={`w-20 h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br ${getMedalStyle(2).bg} flex items-center justify-center font-display font-bold text-2xl text-background ring-4 ${getMedalStyle(2).ring}`}
          >
            {top3[1].avatar}
          </div>
          ...
          <div className="font-display text-display-sm text-gray-300 mt-1">
            #2
          </div>
        ...
        >
          ...
          <div
            className={`w-24 h-24 mx-auto mb-4 rounded-2xl bg-gradient-to-br ${getMedalStyle(1).bg} flex items-center justify-center font-display font-bold text-3xl text-background ring-4 ${getMedalStyle(1).ring}`}
          >
            {top3[0].avatar}
          </div>
          <h3 className="font-display text-heading-lg gradient-text-gold">
            {top3[0].username}
          </h3>
          <div className="font-display text-display-md text-secondary-glow mt-1">
            #1
          </div>
          <div className="flex justify-center gap-4 mt-4 text-body-sm">
            <div>
              <div className="text-text-muted">Wins</div>
              <div className="font-display text-secondary-glow">
                {top3[0].wins}
              </div>
            </div>
            <div>
        ...
        >
          ...
          <div
            className={`w-20 h-20 mx-auto mb-4 rounded-2xl bg-gradient-to-br ${getMedalStyle(3).bg} flex items-center justify-center font-display font-bold text-2xl text-background ring-4 ${getMedalStyle(3).ring}`}
          >
            {top3[2].avatar}
          </div>
          <h3 className="font-display text-heading-md text-text-primary">
          ...
          <div className="font-display text-display-sm text-amber-600 mt-1">
            #3
          </div>
          <div className="flex justify-center gap-4 mt-4 text-body-sm">
            <div>
          ...
          {rest.map((player, index) => (
            ...
            >
              <div className="col-span-2 md:col-span-1 flex items-center gap-2">
                <span className="font-display text-heading-md text-text-secondary">
                  {player.rank}
                </span>
              ...
              <div className="col-span-7 md:col-span-3 flex items-center gap-3 min-w-0">
                ...
                <span className="font-medium text-text-primary truncate">
                  {player.username}
                  {player.isYou && (
                    <span className="text-primary-glow text-body-sm ml-2">
                      (You)
                    </span>
                  )}
                </span>
              ...
              <div className="hidden md:block col-span-2 text-center">
                <span
                  className={`font-display ${getWinRate(player) >= 60 ? "text-accent-green" : getWinRate(player) >= 40 ? "text-secondary-glow" : "text-accent-red"}`}
                >
                  {getWinRate(player)}%
                </span>
              </div>
              ...
              <div className="col-span-2 md:col-span-2 text-center">
                <span className="font-display text-primary-glow">
                  {player.rating}
                </span>
              </div>
            </motion.div>
