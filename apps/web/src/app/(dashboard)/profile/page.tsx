...
export default function DashboardProfilePage() {
  ...
  const achievements = [
    ...
    {
      name: "High Roller",
      desc: "Bet 10,000 coins in one game",
      icon: Wallet,
      unlocked: false,
      progress: 65,
    },
    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-2">
              <h2 className="font-display text-heading-xl gradient-text">
                PlayerOne
              </h2>
      ...
      >
        {stats.map((stat, index) => (
          ...
          >
            ...
            <div className="font-display text-display-sm mb-1">
              {stat.value}
            </div>
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h3 className="font-display text-heading-md mb-1">
              Invite Friends
            </h3>
            <p className="text-text-secondary text-body-sm">
              Share your code, earn 500 coins per referral!
            </p>
          </div>
      ...
      >
        <h2 className="font-display text-heading-lg mb-6 gradient-text">
          Achievements
        </h2>
          ...
          {achievements.map((achievement, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${achievement.unlocked ? "bg-gradient-to-br from-accent-gold to-secondary-glow" : "bg-surface-tertiary"}`}
                >
                  <achievement.icon
                    className={`w-7 h-7 ${achievement.unlocked ? "text-text-inverse" : "text-text-muted"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <h3
                    className={`font-display text-heading-sm ${achievement.unlocked ? "text-text-primary" : "text-text-muted"}`}
                  >
                    {achievement.name}
                  </h3>
