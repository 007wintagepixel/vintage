...
export class RoomService {
  ...
  async createRoom(userId: string, data: CreateRoom) {
    ...
    if (data.entryFee > 0) {
      try {
        await this.gameService["lockFunds"](
          userId,
          data.entryFee,
          "room_entry",
          room.id,
        ...
        this.logger.warn(
          `Failed to lock entry fee for user ${userId}: ${err.message}`,
        );
      }
    }
  ...
  async leaveRoom(userId: string, roomId: string) {
    ...
    if (room.entryFee > 0) {
      try {
        await this.gameService["releaseFunds"](
          userId,
          Number(room.entryFee),
          "room_leave",
          room.id,
        );
      } catch (err) {
        this.logger.warn(
          `Failed to release entry fee for user ${userId}: ${err.message}`,
        );
      }
    }
    ...
    switch (data.action) {
      ...
      case "invite":
        if (!data.inviteUserId)
          throw new BadRequestException("User to invite required");
  ...
  private async invitePlayer(
    ...
    this.logger.log(
      `Room invite sent: room ${roomId}, from ${invitedById} to ${inviteUserId}`,
    );
