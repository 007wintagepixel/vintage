...
export class WalletController {
  ...
  async deposit(@CurrentUser("id") userId: string, @Body() data: Deposit) {
    const idempotencyKey = `deposit-${userId}-${Date.now()}`;

    // Generate payment URL
    const { paymentUrl, transactionId, paymentReference } =
      await this.mockPaymentService.generatePaymentUrl(
        userId,
        data.amount,
        data.paymentMethod,
        data.returnUrl,
  ...
  async completeMockDeposit(
    ...
    return this.mockPaymentService.completeMockPayment(
      data.transactionId,
      data.outcome,
    );
  }
